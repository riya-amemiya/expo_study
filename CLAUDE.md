...
- `bun run start` or `bunx expo start` - Start development server
...
- `bun run lint` - Run ESLint and Biome with auto-fix
- `bun run lint:ci` - Run linting for CI (no auto-fix)
...
This is an Expo project using the new architecture (React Native 0.79.2) with file-based routing via expo-router.
...
- **Biome** - Primary linter/formatter with strict rules
- **ESLint** - Secondary linting via expo config
...
- Bundle ID: `riya-ameiya-lab.com`

### Code Conventions

- Uses Biome with strict rules including:
  - `noDefaultExport: "error"` - Must use biome-ignore comment for required default exports (React components, expo-router pages)
  - Strict TypeScript configuration
  - 80 character line width
  - Space indentation
- Components require default exports for expo-router but must include biome-ignore comment
- Path aliases: Use `@/*` for root-level imports

### Dependencies

- Uses `umt` library for utility functions
- Includes expo-dev-client for development builds
- EAS project ID: `de678e50-dd64-415b-a0a8-d34d0c7af651`
